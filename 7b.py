...
Find the smallest sized directory which satisfies the quiz requirements

Find smallest SizeOfDeletedDir where:
(Total Size - Used Size - SizeOfDeletedDir) > 30000000
...
print(f"Available space is {available_space}")
minsizetodelete = 30000000 - available_space
print(f"Need to delete a dir AT LEAST this size: {minsizetodelete}")

result = list(filter(lambda x: x[1] >= minsizetodelete, root.get_dir_sizes()))
result = list(map(lambda x: x[1], result))

# This is the smallest size of a dir that can be deleted
print(min(result))
